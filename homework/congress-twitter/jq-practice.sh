...
echo '6.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .bio .religion'| sort | uniq -c | sort -rn
echo '7.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | grep -c '2015'
echo '8.'
cat data-hold/legislators-current.json | jq --raw-output '.[] .terms[0] .start' | sort | cut -d '-' -f 1 | head -n 1
echo '9.'
...
echo '13.'
cat data-hold/legislators-social-media.json | jq '.[] .social .faceboook' | grep -v 'null' | wc -l
echo '14.'
cat data-hold/legislators-social-media.json | jq --raw-output '.[]  | [.id .bioguide, .social .twitter] | @csv' | head -n 10
echo '15.'
cat data-hold/legislators-social-media.json |  jq --raw-output '.[]  | [.id .bioguide, .social.twitter] | @csv'| grep -vE '"[[:alpha:]][[:digit:]]{6}","'
echo '16.'
...
jq --raw-output '.[] | [.name, .screen_name, .followers_count, .verified, .created_at] | @csv' | head -n 10
echo '19.'
cat data-hold/congress-twitter-profiles.json | \
